...
void Image::savebmp(const char* filename)
{
	...
	for (int i = 0; i < height; i++)
	{
		...
		{
			img[counter] = (unsigned char)getPixel(j, i).z;
			img[counter + 1] = (unsigned char)getPixel(j, i).y;
			img[counter + 2] = (unsigned char)getPixel(j, i).x;
			counter = counter + 3;
		}
	}
	...
	long siz = height * width;
	//fwrite(pixels, 3, siz, f);
	for (i = 0; i < height; i++)
	{
